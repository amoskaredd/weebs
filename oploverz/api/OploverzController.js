...
class OploverzController {
  async searchOld({ query: { query } }, req) {
    ...
    await axios.get(host, config).then((res) => {
      const html = res.data;
      ...
      req.send({ message: "success", length: index.length, result: index })
        .catch((err) => {
          req.send('something error from server')
        })
    });
    // console.log(result)
  }
  ...
  async search(query, req) {
    ...
    data.anime[0]["all"].forEach(function(v) {
      ...
      //return ({message: 'success', length: data.anime[0]['all'].length, result})
      //console.log(id)
    });

    return { message: "success", length: index.length, result: index };
  }


  async detail({ query: { url } }, req) {
    //let id = request.input("id");
    const cacheDetail = nodeCache.has('detail' + url)

    if (cacheDetail) {
      console.log('Mengambil data dari cache')
      return nodeCache.get('detail' + url)
    }
    const config = {
      headers: {
        "user-agent":
    ...
    await axios.get(url, config).then((res) => {
      ...
      req.send({ message: "success", length: index.length, result: index })
        .catch((err) => {
          req.json('something error from server')
        })
    });
    //console.log(result)
  }

  async download({ query: { url } }, req) {
    //let eps = request.input("id");
    const cacheDownload = nodeCache.has(url)
  ...
  async latestCache(_, req) {
    ...
    list.each((i, el) => {
      ...
      const id = $(el)
        ...
        .match(/(?<=anime\/)(.*)/g)[0]
        .replace("/", "");
      result.push({ title, id, status, type, poster, link });
    });

    // Simpan data ke cache
    cache[url] = {
      data: { message: "success", length: result.length, result },
      expiry: Date.now() + 3600 * 1000,
    };
    //console.log(result)
    return { message: "success", length: result.length, result };
  }


  async latest_update(_, req) {
    const config = {
      headers: {
        "user-agent":
          "Mozilla/5.0 (Linux; Android 12; CPH2043) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36",
